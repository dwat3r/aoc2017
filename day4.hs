...
s = ["sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp",
...
part1 = length $ filter (\ss -> length (nub ss) == length ss) $ map words s

part2 = length $ filter go $ map words s
  where
    go ss = length (nub sss) == length sss
      where
        sss = map sort ss